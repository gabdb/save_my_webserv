...
/*   Updated: 2026/01/13 13:32:08 by gnyssens         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
TCPserver::TCPserver(const Config &cfg) : _config(cfg) {}
...
void TCPserver::initListeners() {
    ...
    for (size_t i = 0; i < servers.size(); ++i) {
        ...
        std::vector<std::string>
            hosts; // CHANGEMENT GAB, pcq GetStringValues throw error si pas de
                   // host dans .config
        try {
        ...
        // Check if "host:port" is allready in map. If yes, append to key's
        // server list. Otherways, create mapping
        for (size_t h = 0; h < hosts.size(); ++h) {
            for (size_t p = 0; p < ports.size(); ++p) {
                ...
                std::map<std::string, Listener>::iterator it =
                    listenerMap.find(key);
    ...
    for (std::map<std::string, Listener>::iterator it = listenerMap.begin();
         it != listenerMap.end(); ++it)
...
void TCPserver::createListeningSockets() {
    for (size_t i = 0; i < _listeners.size(); ++i) {
        ...
        if (bind(fd, reinterpret_cast<struct sockaddr *>(&addr), sizeof(addr)) <
            0)
            throw std::runtime_error("bind() failed on " + lst.host + ":" +
                                     intToStr(lst.port));
        if (listen(fd, SOMAXCONN) < 0)
            throw std::runtime_error("listen() failed on " + lst.host + ":" +
                                     intToStr(lst.port));
        if (fcntl(fd, F_SETFL, O_NONBLOCK) < 0)
            throw std::runtime_error(
                "fcntl(O_NONBLOCK) failed on listening socket");

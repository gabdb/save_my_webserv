...
Client &CGI::getClient() const { return *this->_client; }

const std::string &CGI::getRawOutput() const { return this->_fromCgi; }

int CGI::getStdoutFd() const { return _stdoutFd; }

bool CGI::isFinished() const { return this->_finished; }

bool CGI::hasError() const { return this->_error; }

int CGI::getExitStatus() const { return this->_exitStatus; }

static std::string buildPathInfo(const std::string &requestTarget,
                                 const std::string &scriptPath) {
    (void)scriptPath;
    std::string::size_type q = requestTarget.find('?');
    std::string pathOnly =
        (q == std::string::npos) ? requestTarget : requestTarget.substr(0, q);
    return pathOnly;
}

std::vector<std::string> buildCgiEnv(const Client &client,
                                     const std::string &scriptPath,
    ...
    // TODO: Implement logic to store host address and port of VServer. (will
    // depend on how Nico/Gab store final values (maybe key:value or seperate
    // member?))
...
CGI::CGI(Client &client, const std::string &interpreterPath,
...
void CGI::startProcess(const std::string &interpreterPath,
    ...
    if (pid == 0) {
        ...
        std::vector<std::string> envStrings =
            buildCgiEnv(*_client, scriptPath, queryString);
...
bool CGI::ownsFd(int fd) const { return (fd == _stdinFd || fd == _stdoutFd); }
...
void TCPserver::handleCgiIO(int fd, short revents) {
    ...
    if (cgi->isFinished()) {
        ...
        if (err) {
            ...
            buildAndSendCgiResponse(cl, out);
        }
    }
}
